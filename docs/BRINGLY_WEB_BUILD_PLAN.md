...
- [x] 12 — OTP login UI — `/login` (noindex, Suspense/CSR): phone step (+91, 10-digit, `sendOtp` via BFF with backend limits surfacing in Hindi), OTP step (OtpInput = single invisible input over 6 boxes — paste/backspace/`one-time-code` autofill just work; auto-verify on 6th digit; 30s resend cooldown; नंबर बदलें), verify via cookie-minting `/api/auth/verify-otp` (NEVER the raw backend), open-redirect-guarded `next`, already-authed → redirect, on success: `refresh()` session → `replayGuestCart` → clear local → `router.replace(next)`. Verified: typecheck+build ✅, send-otp through BFF vs live backend ✅, replay path (login→cookies→BFF POST /cart/items) puts real product in server cart ✅.
- [x] 13 — COD checkout (gated) — `/checkout` (noindex, middleware-gated): **flushes any guest-cart lines into the server cart on entry** (tiles keep writing to the guest cart post-login; checkout is the sync point, mirroring login replay), AddressPicker (radio + डिफ़ॉल्ट badge + inline AddressForm: label chips, geo-autocomplete locality w/ per-mount sessionToken → placeDetails lat/lng/pincode, 📍 GPS + reverseGeocode, pincode 333026 gate, Chirawa-centre coord fallback, receiver fields), read-only server-cart summary, BillSummary (fee+distance, मुफ्त, discount+promo code, कुल, Hindi `breakdownText`), place order via **raw same-origin fetch with `Idempotency-Key`** (api-client can't set headers; BFF forwards it), `PRICING_REFRESH_REQUIRED` → refetch cart+preview, success → `/order/[id]?group=`. No payment UI anywhere. Verified live: address→preview (fee ₹10, FIRSTORDER −₹10)→order `confirmed` ₹570, **no razorpay fields**, double-submit same key → same orderId ✅.
- [x] 14 — Order confirmation + live tracking (gated) — `/order/[orderId]` (+`?group=`): `useOrderSocket` (socket.io-client added to apps/web deps — root dep isn't importable under pnpm isolation; token via auth **callback** so reconnects refetch `/api/auth/socket-token`; order:subscribe/unsubscribe; handlers in a ref), 15s poll fallback (`refetchInterval` off on terminal), confirmation header (लाइव/poll badge, COD due incl. `cash_adjustment` refund note), StatusStepper (5 phases; cancelled → banner), EtaHero (skew-safe countdown, spread range), RiderCard (call + maps link; initial `getRiderLocation`), item-unavailable notices, items+bill, address, cancel (confirmed/preparing via **DELETE /orders/:id** — BFF allowlist fixed: cancel is DELETE not POST, rating path is `/rating` not `/rate`), 5-star rating on delivered, GroupSummary (multi-shop siblings). Verified live: anon 307 gate ✅ authed 200 ✅ rating→backend business-rule (not 404) ✅ cancel→`cancelled` ✅; also exercised refresh-rotation theft-detection (stale jar rt correctly rejected). Socket live-push pending Task 16 CORS.
- [x] 15 — Orders history + Account/Addresses (gated) — `/orders`: status-chip cards (Hindi map), items summary, ट्रैक/देखें + फिर से ऑर्डर करें (replays delivered order lines into the guest cart → /cart). **Gotcha: `GET /orders` list returns a raw row (`totalAmount`, items without subtotal) — api-client's `OrderDetailResponse[]` type is wrong for this endpoint**; local `OrderListRow` type. `/account`: session badge, मेरे ऑर्डर/मेरे पते links, hi/EN language toggle (`useLanguage`), WhatsApp help, logout (revoke+clear → /). `/account/addresses`: list + AddressForm reuse, हटाएं, डिफ़ॉल्ट बनाएं (**PATCH `/users/me/addresses/:id/default` — added to BFF PATCH allowlist; the POST variant doesn't exist backend-side and was removed**). Header: 👤 → /account. Verified live: 3×307 anon gates ✅ 3×200 authed ✅ set-default "Default address set" ✅ list consumed ✅.
- [x] 16 — (Backend) Socket.IO CORS — **zero code change needed**: `realtime.plugin.ts` already allows `'*'` in dev and locks prod to `env.FRONTEND_URLS` (which defaults to `http://localhost:3001` — the web port). Socket auth (JWT middleware) + `order:subscribe` IDOR guard already in place. Verified live from web deps: `/api/auth/socket-token` → `io(localhost:3000, {auth:{token}, transports:['websocket']})` → `connected` as customer → subscribe → clean disconnect ✅. **Deploy note: prod `FRONTEND_URLS` must include the real web origin (e.g. https://chirawa.in).**
- [x] D1 (security, backend) — trustProxy configurable — `TRUST_PROXY` env (zod-transformed: true/false/hop-count/CIDR list, default `true`) wired into `app.ts` (was hardcoded `true` = any direct caller could spoof X-Forwarded-For rate-limit keys). Prod guidance in `.env.example`: set to real proxy hops (`1` or `127.0.0.1,::1`). Context7-checked against Fastify v4 trustProxy semantics. Verified: API reboots clean, web→BFF→API 200 ✅.
