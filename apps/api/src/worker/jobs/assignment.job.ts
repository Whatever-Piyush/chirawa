...
export async function processAssignBatch(
  ...
  if (result.assigned) {
    logger.info({ batchId, riderId: result.riderId, orderCount: result.orderCount }, `🛵 [assign] batch ${batchId} -> rider ${result.riderId}`);
    return;
  }
  if (result.reason === 'already_handled' || result.reason === 'empty' || result.reason === 'not_found') {
    logger.info({ batchId, reason: result.reason }, `🛵 [assign] batch ${batchId} skipped`);
    return;
  }
  ...
  if (attempt < MAX_ATTEMPTS) {
    ...
    logger.info({ batchId, attempt, maxAttempts: MAX_ATTEMPTS, retryInMs: RETRY_DELAY_MS }, `🛵 [assign] batch ${batchId} no rider — retrying`);
    return;
  }
...
async function escalate(prisma: PrismaClient, batchId: string): Promise<void> {
  ...
  logger.error({ batchId }, `🛵 [assign] batch ${batchId} UNASSIGNED after max attempts — escalating`);
  if (cfg?.value) {
    await sendSms(
      cfg.value,
      `Bringly: Ek batch (${batchId.slice(-6).toUpperCase()}) ko koi rider nahi mila. Admin app me manually assign karein.`,
    ).catch((err) => logger.error({ err, batchId }, 'Escalation SMS failed'));
  }
}
