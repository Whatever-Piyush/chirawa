...
type Props = { navigation: NativeStackNavigationProp<RootStackParamList, 'VerifyOtp'>; route: RouteProp<RootStackParamList, 'VerifyOtp'> };
export default function VerifyOtpScreen({ route }: Props) {
  ...
  async function handleVerify() {
    ...
    setLoading(true);
    try {
      const res = await RiderApi.verifyOtp(phone, otp);
      await StorageService.setTokens({ accessToken: res.tokens.accessToken, refreshToken: res.tokens.refreshToken, expiresIn: 900 });
      const p = JSON.parse(atob(res.tokens.accessToken.split('.')[1] ?? '')) as { sub: string };
      signIn(res.tokens.accessToken, p.sub, res.requiresPin);
    } catch (e: unknown) { Alert.alert('Error', e instanceof Error ? e.message : 'Galat OTP'); setOtp(''); }
