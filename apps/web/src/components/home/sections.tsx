  ...
  return (
    <section className="mt-8">
      <div className="mb-3 flex items-end justify-between gap-2">
        <h2 className="text-xl font-heavy text-ink">{title}</h2>
        {action}
      </div>
      {children}
    </section>
          ...
          return (
            ...
            >
              <div className="relative h-16 w-16 overflow-hidden rounded-full bg-primary-light">
                {img ? (
                  <Image src={img} alt={c.name} fill sizes="64px" className="object-cover" />
                ) : (
              ...
              <span className="line-clamp-2 text-center text-xs font-medium text-ink">{c.name}</span>
            </Link>
          );
        })}
      </div>
          ...
          >
            <div className="grid grid-cols-2 gap-1.5">
              {cluster.images.slice(0, 4).map((url, i) => (
                <div key={i} className="relative aspect-square overflow-hidden rounded-md bg-surface-alt">
                  <Image src={url} alt="" fill sizes="80px" className="object-contain" />
                </div>
              ))}
            </div>
            <p className="mt-2 text-sm font-bold text-ink">{cluster.name}</p>
          </Link>
        ))}
      </div>
    </Section>
